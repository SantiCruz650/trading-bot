...
function displayBacktestResults(data) {
    ...
    const profit = data.total_return !== undefined ? (data.total_return * 100).toFixed(1) + '%' : 'N/A';
    const sharpe = data.sharpe_ratio !== undefined ? data.sharpe_ratio.toFixed(2) : 'N/A';
    const maxDd = data.max_drawdown !== undefined ? (data.max_drawdown * 100).toFixed(1) + '%' : 'N/A';
...
// --- ETAPA 2A: Risk Containment Logic ---

let etapaInterval = null;

function startEtapaMonitoring() {
    if (etapaInterval) clearInterval(etapaInterval);
    fetchEtapa2AStatus(); // Initial fetch
    etapaInterval = setInterval(fetchEtapa2AStatus, 5000); // Poll every 5s
}

function stopEtapaMonitoring() {
    if (etapaInterval) {
...
async function fetchEtapa2AStatus() {
    if (!authToken) return;

    try {
        const response = await fetch(`${API_URL}/trading-live/risk/gec-status`, {
            headers: { 'Authorization': `Bearer ${authToken}` }
        });

        if (response.ok) {
            const data = await response.json();
            updateEtapaPanel(data);
        }
    } catch (error) {
        console.error('Failed to fetch ETAPA 2A status:', error);
    }
}

function updateEtapaPanel(data) {
    const panel = document.getElementById('etapa-2a-panel');
    if (!panel) return;

    // 1. Exposure Ratio
    const erEl = document.getElementById('etapa-er');
    if (erEl) {
        erEl.textContent = (data.exposure_ratio * 100).toFixed(1) + '%';
        // Color coding
        if (data.exposure_ratio >= data.thresholds.hard_cap) erEl.className = 'stat-value text-danger';
        else if (data.exposure_ratio >= data.thresholds.soft_cap) erEl.className = 'stat-value text-warning';
        else erEl.className = 'stat-value text-success';
    }

    // 2. Daily Drawdown
    const ddEl = document.getElementById('etapa-dd');
    if (ddEl) {
        ddEl.textContent = data.daily_drawdown_pct.toFixed(2) + '%';
        // Color coding
        if (data.daily_drawdown_pct >= data.thresholds.freeze_dd) ddEl.className = 'stat-value text-danger';
        else if (data.daily_drawdown_pct >= 1.0) ddEl.className = 'stat-value text-warning';
        else ddEl.className = 'stat-value text-success';
    }

    // 3. GEC State Badge
    const badgeEl = document.getElementById('etapa-gec-badge');
    const adjEl = document.getElementById('etapa-adjustments');

    if (badgeEl) {
        badgeEl.textContent = data.gec_state;

        if (data.gec_state === 'NORMAL') {
            badgeEl.className = 'badge bg-success';
            if (adjEl) adjEl.innerHTML = '<span class="text-muted">No active adjustments</span>';
        } else if (data.gec_state === 'SOFT_CAP') {
            badgeEl.className = 'badge bg-warning text-dark';
            if (adjEl) adjEl.innerHTML = `
                <div class="text-warning"><i class="fas fa-compress-arrows-alt me-1"></i>Size: -${data.adjustments.order_size_reduction_pct.toFixed(0)}%</div>
                <div class="text-warning"><i class="fas fa-clock me-1"></i>DCA: +${data.adjustments.dca_step_increase_pct.toFixed(0)}%</div>
            `;
        } else if (data.gec_state === 'HARD_CAP') {
            badgeEl.className = 'badge bg-danger';
            if (adjEl) adjEl.innerHTML = '<div class="text-danger fw-bold"><i class="fas fa-ban me-1"></i>BUY BLOCKED</div>';
        } else if (data.gec_state === 'KILL_SWITCH') {
            badgeEl.className = 'badge bg-danger blink-me'; // Add CSS for blink if desired
            if (adjEl) adjEl.innerHTML = '<div class="text-danger fw-bold">SYSTEM HALTED</div>';
        }
    }

    // 4. System Status Indicators
    const freezeStatus = document.getElementById('etapa-freeze-status');
    const killStatus = document.getElementById('etapa-killswitch-status');
    const normalStatus = document.getElementById('etapa-normal-status');
    const freezeReason = document.getElementById('etapa-freeze-reason');

    // Reset visibility
    if (freezeStatus) freezeStatus.classList.add('d-none');
    if (killStatus) killStatus.classList.add('d-none');
    if (normalStatus) normalStatus.classList.add('d-none');

    if (data.kill_switch_active) {
        if (killStatus) killStatus.classList.remove('d-none');
        // Flash the panel border red
        panel.style.border = '2px solid #ef4444';
        panel.style.boxShadow = '0 0 20px rgba(239, 68, 68, 0.3)';
    } else if (data.gec_state === 'HARD_CAP' || (data.gec_state !== 'NORMAL' && data.gec_state !== 'SOFT_CAP')) {
        // Fallback for freeze if not explicitly in GEC state but freeze_active is true (handled below)
    }

    // Check freeze separately as it can coexist with other states (though usually overrides)
    // The API returns freeze_active in the root of the response? No, it's not in the root of get_gec_status response in my implementation plan?
    // Wait, I put it in get_risk_stats, but did I put it in get_gec_status?
    // Let's check the implementation of get_gec_status in trading_live.py
    // I returned: "gec_state", "soft_cap_active", "hard_cap_active", "kill_switch_active".
    // I missed "freeze_active" in get_gec_status! I put it in get_risk_stats.
    // However, I can infer freeze from the GEC state if I mapped it, OR I should update the API.
    // Actually, I should update the API to include freeze_active in get_gec_status for completeness.
    // But for now, let's use what we have.
    // Wait, I can fetch get_risk_stats as well, or just update get_gec_status.
    // Let's update get_gec_status in the backend quickly to be sure.

    // Actually, looking at my previous tool output for trading_live.py:
    // I added get_gec_status but I didn't include freeze_active in the return dict explicitly, 
    // I only included it in get_risk_stats.
    // BUT, I can use get_risk_stats to populate this too if I want.
    // Let's stick to get_gec_status. I'll assume for now I'll update the backend to include it.

    // For now, let's assume the backend sends it or I'll update the backend.
    // I will update the backend to include freeze_active in get_gec_status.

    if (normalStatus && !data.kill_switch_active && data.gec_state === 'NORMAL') {
...
showSection = function (section) {
    ...
    if (section === 'dashboard') {
        startEtapaMonitoring();
    } else {
        stopEtapaMonitoring();
    }
};

// Start if already on dashboard
